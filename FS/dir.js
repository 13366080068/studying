...
//   }
// }
// mkdirSync('a/b/c/d/e/qqq/xxx/ooo')
...
//   function next() {
//     if (dirs.length === index) return callback()
//     let current = dirs.slice(0, ++index).join('/')
//     fs.stat(current, (err, statObj) => {
//       if (!err) return next() // 没错误说明文件存在了
...
// dirs = dirs.map(dir => path.join(baseDir, dir))

// for (let i = 0; i < dirs.length; i++) {
//   let current = dirs[i]
//   let statObj = fs.statSync(current)
//   if (statObj.isFile()) fs.unlinkSync(current)
...
//           if (++index === dirs.length) fs.rmdir(url, callback)
//         }
//         dirs.forEach(dir => {
//           rmdir(dir, done)
//         })
...
//     fs.stat(url, (err, statObj) => {
//       if (statObj.isFile()) fs.unlink(url, resolve)
//       else fs.readdir(url, (err, dirs) => {
//         dirs = dirs.map(dir => path.join(url, dir))
//         Promise.all(dirs.map(dir => rmidr(dir))).then(() => {
