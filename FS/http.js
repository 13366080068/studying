...
// 请求方法 get(获取资源) post(新增资源) put(上传文件) delete options(跨域请求的时候 非简单请求时才会出现 (get和post 都是简单请求 + 自定义的头信息))
...
let server = http.createServer((req, res) => { // 当请求到来时会执行此方法
...
server.on('error', err => {
  if (err.errno === 'EADDRINUSE') server.listen(++port) // 如果端口号被占用了 直接重新报监听新的端口号即可
})
